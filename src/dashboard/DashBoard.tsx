...
export default function DashBoard() {
    ...
    const fetchTrader = async (): Promise<void> => {
        const url: string = `http://0.0.0.0:3000/api/traders/${localStorage.getItem("traderId")}`;
    ...
    const fetchPortfolio = async(): Promise<void> => {
        const url = `http://0.0.0.0:3000/api/traders/${localStorage.getItem("traderId")}`;
    ...
    const generateTIckerToken = async(): Promise<void> => {
        const url = `http://0.0.0.0:3000/api/ticker_tokens?ticker[symbol]=${ticker}`;
    ...
    const transact = async (price: number): Promise<void> => {
        const url = `http://0.0.0.0:3000/api/tickers/${localStorage.getItem("tickerId")}`;
    ...
    const acquireStock = async (): Promise<void> => {
        const url = "http://0.0.0.0:3000/api/tickers";
    ...
    const generateTransactionHistory = async(mode: "buy" | "sell"): Promise<void> => {
        const url = "http://0.0.0.0:3000/api/transactions";
